...
class SWINSightTrain:
    def __init__(self, pretrain: bool = True, size:Literal['base', 'large'] = 'large') -> None:
        ...
        self._train_len = len(os.listdir('./dataset/train'))
        self._val_len = len(os.listdir('./dataset/validation'))
    ...
    def _get_train_dataset_loaders(self):
        train_images, train_labels = get_images(dataset_path='./dataset/train', transform=self._transform)

        val_images, val_labels = get_images(dataset_path='./dataset/validation', transform=self._transform)
    ...
    def _pretrain_swin(self, epochs: int, lr: float):
        ...
        for i in range(epochs):
            ...
            torch.save(self._model,f"./model_history/{save_str}_{i}")
            if validation_loss < best_val_loss:
                best_val_loss = validation_loss
                time_till_best = 0
                best_model = f"./model_history/{save_str}_{i}"
                continue
            time_till_best+=1
    ...
    def train_model(self):
        ...
        torch.save(self._model,f"./model_history/SWINModelLarge_finetuned_hybrid")